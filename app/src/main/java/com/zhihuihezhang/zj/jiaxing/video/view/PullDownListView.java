package com.zhihuihezhang.zj.jiaxing.video.view;
...
import com.zhihuihezhang.zj.jiaxing.video.util.Utils;
...
public class PullDownListView extends FrameLayout implements GestureDetector.OnGestureListener,
    ...
    private final FlingRunnable mFlinger;
    ...
    private final CheckForLongPress mPendingCheckForLongPress = new CheckForLongPress();

    private final CheckForLongPress2 mPendingCheckForLongPress2 = new CheckForLongPress2();
    ...
    private void addRefreshBar() {
        ...
        // 刷新头部的view
        view = LayoutInflater.from(getContext()).inflate(R.layout.pulldown_refresh_bar, null);
        // 添加view在本控件中
        addView(view);
        ...
        FrameLayout.LayoutParams layoutparams1 = new FrameLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT);
        ...
        head_lastUpdatedTextView = (TextView) view.findViewById(R.id.head_lastUpdatedTextView);
    ...
    protected void onFinishInflate() {
        ...
        foot = (LinearLayout) LayoutInflater.from(getContext()).inflate(R.layout.pulldown_loading, null);
     ...
     */
    private void init() {
        maxLength = getResources().getDimensionPixelSize(R.dimen.updatebar_height);// 62.0dip
    ...
    private boolean move(float deltaY, boolean auto) {
        // move 方法执行 "
        if (deltaY > 0 && mFirstChild.getTop() == -maxLength) {
            mPading = -maxLength;
            return false;
        }
        ...
        if (deltaY > 0 && mFirstChild.getTop() <= -maxLength) {
            mPading = -maxLength;
            deltaY = -maxLength - mFirstChild.getTop();
            mFirstChild.offsetTopAndBottom((int) deltaY);
            mListView.offsetTopAndBottom((int) deltaY);
            mPading = mFirstChild.getTop();
            updateView();
    ...
    private void scrollToClose() {
        mDestPading = -maxLength;
        mFlinger.startUsingDistance(maxLength, CLOSEDELAY);
    }
    ...
    public void onRefreshComplete(String date) {
        ...
        head_lastUpdatedTextView.setText(getResources().getString(R.string.header_last_time)
                + Utils.longToStrDate(System.currentTimeMillis()));
        head_lastUpdatedTextView.setTextColor(Color.GRAY);
        updateCommon();
        scrollToClose();
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            case MotionEvent.ACTION_UP:
                ...
                if (!handled && mFirstChild.getTop() == -maxLength && f1 || mState == STATE_REFRESH) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!handled && mFirstChild.getTop() == -maxLength) {
                    try {
    ...
    protected void onLayout(boolean flag, int i, int j, int k, int l) {
        ...
        mFirstChild.layout(0, top, w, top + maxLength);

        int h = getMeasuredHeight() + mPading + maxLength;
        mListView.layout(0, top + maxLength, w, h);
    }
    ...
    public boolean onScroll(MotionEvent curdown, MotionEvent cur, float deltaX, float deltaY) {
        ...
        if (deltaY < 0F && flag || getChildAt(0).getTop() > -maxLength) { // deltaY
                                                                          // <
                                                                          // 0
                                                                          // 向下
